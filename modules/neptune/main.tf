...
resource "aws_neptune_parameter_group" "this_parameter_group" {
  for_each    = { for k, values in var.core-db-parameter-groups : k => values.id if(values.cluster == false) }
  name        = each.value.name
  description = each.value.description
...
resource "aws_neptune_cluster_parameter_group" "this_cluster_parameter_group" {
  ...
  for_each    = { for k, values in var.core-db-parameter-groups : k => values.id if values.cluster }
  name        = each.value.name
  description = each.value.description
  family      = each.value.family
